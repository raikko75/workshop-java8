...
public class Stream_01_Test {
	...
	public void test_stream_filter() throws Exception {
		...
		List<Pizza> priceSup = pizzas.stream()
		...
		assertThat(priceSup, hasSize(3));
		assertThat(priceSup, everyItem(hasProperty("price", anyOf(equalTo(1300), greaterThan(1300)))));
	}
	...
	public void test_stream_anyMatch() throws Exception {
		...
		List<Pizza> pizzas = new Data().getPizzas();
		Map <Boolean , List<Pizza>	>  map = pizzas.stream().collect(Collectors.partitioningBy(p->p.getPrice() >= 1300));


		// TODO valider si au moins une pizza à un prix >= 1300
		Boolean result1 = pizzas.stream().anyMatch( t -> t.getPrice() >=1300);
		// anymatch renvoie true si la condition est remplie 
	...
	public void test_stream_filter_and_match() throws Exception {
		...
		List<Order> result = orders.stream().filter(p -> p.getCustomer().getFirstname().equals("Johnny"))
				.filter(p -> p.getPizzas().stream().anyMatch(t-> t.getPrice() >= 1300))
				.collect(Collectors.toList());
