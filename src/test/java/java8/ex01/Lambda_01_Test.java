...
public class Lambda_01_Test {
    ...
    private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
    	...
    	ArrayList<Person> personneAge = new ArrayList<Person>();
    	 ArrayList <Person> namePersonne = new ArrayList<Person>();
    	 ArrayList <Person> password = new ArrayList<Person>();
    		for (Person str : personneAge)
    		{
    		...
    		for(Person str : namePersonne)
    		{
    			if(predicate.test(str))
    			{
    				namePersonne.add(str);
    			}
    				
    		}
    		for(Person str : password)
    		{
    			if(predicate.test(str))
    			{
    				password.add(str);
    ...
    public void test_filter_by_age() throws Exception {
        ...
        List<Person> result = filter(personList, (Person p) -> p.getAge()>=18 ); //  expression lambda  parametre l'objet personne -> conditions
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = filter(personList, ((Person p) -> p.getAge() >= 49 && DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex)));
        		//(p.getPassword().equals(passwordSha512Hex))));
